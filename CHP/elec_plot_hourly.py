...
electricity_market = markets["elec"].to_numpy() * 0.0000001
...
def pyomomodel(total_hours = total_hours, time_limit = time_limit, CHP_capacity=CHP_capacity, energy_ratio = energy_ratio):
    ...
    model.elec_to_heat = Var(model.HOURS, within=NonNegativeReals)
...
# ======== CO2 Constraints ========
    ...
    def elec_for_heat_conversion_rule(model, h):
        ...
        return model.heat_production[h] >= model.useful_heat[h] + elec_to_heat_efficiency * model.elec_to_heat[h] * model.use_elec_for_heat
    ...
    def updated_elec_demand_balance_rule(model, h):
        return model.elec_to_plant[h] + model.purchased_electricity[h] == electricity_demand[h] - model.elec_reduction_by_CHP[h] + model.elec_to_heat[h]
    ...
    def updated_heat_demand_balance_rule(model, h):
        return model.heat_to_plant[h] == heat_demand[h] - model.heat_reduction_by_CHP[h] + elec_to_heat_efficiency * model.elec_to_heat[h] * model.use_elec_for_heat
    ...
    def objective_rule(model):
        ...
        elec_to_heat_cost = sum(model.elec_to_heat[h] * electricity_market[h] * model.use_elec_for_heat for h in model.HOURS)


        return (
            (fuel_cost_NG + fuel_cost_H2 + fuel_cost_BM) + elec_cost + carbon_cost + all_shortfall_penalty + elec_to_heat_cost
