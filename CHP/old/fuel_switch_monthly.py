...
markets = pd.read_excel(markets_path, nrows=100000)
...
H2_market = markets["hydrogen"].to_numpy() * 1
...
def pyomomodel(total_months = total_months, time_limit = time_limit, CHP_capacity=CHP_capacity, energy_ratio = energy_ratio):
    ...
    M = 1E8
        ...
        # Parameters related to the CO2 stream
    co2_stream_temp = 400  # Example: CO2 stream temperature (in Kelvin), can be varied
    co2_stream_pressure = 10  # Example: CO2 stream pressure (in bar), can be varied
    co2_concentration = 0.2  # CO2 concentration in the flue gas stream, can be varied
