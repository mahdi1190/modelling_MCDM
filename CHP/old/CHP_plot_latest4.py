...
# Extracting data from the loaded DataFrames
...
BM_market = markets["biomass"].to_numpy() * 1.5
...
app.layout = html.Div([
    ...
    dcc.Graph(id='eff-graph'),  # New graph for efficiency
    dcc.Graph(id='carbon-credits'),  # New graph for carbon credits
    dcc.Graph(id='fuel_blend'),  # New graph for fuel blend
...
def update_graphs(n_intervals):
    ...
    try:
        current_mod_time = os.path.getmtime(r"C:\Users\fcp22sma\modelling_MCDM\CHP\CHP_plot_latest4.py")
    except FileNotFoundError as e:
    ...
    if current_mod_time > last_mod_time:
        ...
        # Assuming you have a function 'pyomomodel' to build and solve the model
        model = pyomomodel()  # Rebuild the model
        hours_list = list(model.HOURS)
        intervals_list = list(model.INTERVALS)

        # Example data extraction, replace with your actual data processing logic
        ...
        # Create figures (these should match your data)
        electricity_figure = {
            'data': [
                ...
                {'x': list(model.HOURS), 'y': [purchased_electricity[h] for h in list(model.HOURS)], 'line': {'width': 3, 'dash': 'dash', 'color': 'hydrogen'}, 'name': 'Purchased Electricity'},
        ...
        # Return the updated figures and text elements
        return (electricity_figure, heat_figure, cold_figure, heat_storage, purchased_elec, eff_figure, 
                carbon_credits_figure, fuel_blend_figure, 
