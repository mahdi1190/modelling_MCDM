...
H2_market = markets["hydrogen"].to_numpy() * 200
...
def update_graphs(n_intervals):
    ...
    current_mod_time = os.path.getmtime(r"C:\Users\fcp22sma\modelling_MCDM\CHP\CHP_plot_latest5.py")
    ...
    if current_mod_time > last_mod_time:
        ...
        for i in model.INTERVALS:
            if value(model.invest_time[i]) > 0.5:  # Check if the binary variable is 1
                print(f"Investment in hydrogen is made at interval {i}, which corresponds to month {i * i // (24*30)}.")
...
def pyomomodel(total_hours = total_hours, time_limit = time_limit, CHP_capacity=CHP_capacity, energy_ratio = energy_ratio):
    ...
    h2_capex = 150000  # Example value, adjust based on your case
    ...
    model.active_h2_blending = Var(model.HOURS, within=Binary)  # To activate H2 blending after investment
 ...
 # -------------- Constraints --------------
    ...
    def h2_blending_activation_rule(model, h):
        return model.fuel_blend_h2[h] <= model.active_h2_blending[h]
    ...
    def active_h2_blending_rule(model, h):
        return model.active_h2_blending[h] == sum(model.invest_time[i] for i in model.INTERVALS if i * intervals_time <= h)
    model.active_h2_blending_constraint = Constraint(model.HOURS, rule=active_h2_blending_rule)
...
# ======== CO2 Constraints ========
    ...
    def objective_rule(model):
        ...
        carbon_sold = sum(model.credits_sold[i] * carbon_market[i] for i in model.INTERVALS) * 0.4
