...
def pyomomodel(total_months = total_months, time_limit = time_limit, CHP_capacity=CHP_capacity, energy_ratio = energy_ratio):
    ...
    model.electrical_efficiency = Var(model.MONTHS, within=NonNegativeReals, bounds=(0.99, 1))
    model.thermal_efficiency = Var(model.MONTHS, within=NonNegativeReals, bounds=(0.99, 1))
    ...
    # Market and Storage
    model.purchased_electricity = Var(model.MONTHS, within=NonNegativeReals, bounds=(0, max(electricity_demand)))
 ...
 # -------------- Constraints --------------
    ...
    def heat_demand_balance(model, m):
        return model.heat_to_plant[m] == heat_demand[m]
    ...
    # Ramp Up
    def ramp_up_rule(model, m):
        if m == 0:
            return Constraint.Skip  # Skip the first month
        return model.heat_production[m] - model.heat_production[m - 1] <= model.ramp_rate[m]
    model.ramp_up_constraint = Constraint(model.MONTHS, rule=ramp_up_rule)

    # Ramp Down
    def ramp_down_rule(model, m):
        if m == 0:
            return Constraint.Skip  # Skip the first month
        return model.heat_production[m - 1] - model.heat_production[m] <= model.ramp_rate[m]
    model.ramp_down_constraint = Constraint(model.MONTHS, rule=ramp_down_rule)

    # Max Ramp
    def max_ramp_rule(model, m):
        return model.ramp_rate[m] <= max_ramp_rate
    model.max_ramp_constraint = Constraint(model.MONTHS, rule=max_ramp_rule)
...
# ======== CO2 Constraints ========
    ...
    def co2_emissions_rule(model, m):
        ...
        return model.co2_emissions[m] == (
            ...
            co2_per_unit_elec * model.purchased_electricity[m]
        )
    ...
    def objective_rule(model):
        elec_cost = sum(model.purchased_electricity[m] * electricity_market[m] for m in model.MONTHS)
